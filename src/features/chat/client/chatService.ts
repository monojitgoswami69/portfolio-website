// Chat API Service - Connected to the app's own integrated chat API
...
// Temporary offline responses if the local AI core is unavailable
const getOfflineResponse = (message: string): string => {
  const lowerMessage = message.toLowerCase();

  if (lowerMessage.includes('hello') || lowerMessage.includes('hi')) {
    return "**NEXUS ONLINE**\n\n> Greetings! I'm currently in `OFFLINE_MODE` because the local AI core is unavailable.\n\nOnce it is back, I'll answer questions about **Monojit's** skills, projects, and experience.";
  }
